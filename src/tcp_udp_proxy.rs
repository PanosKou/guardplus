...
pub async fn run_tcp_gateway(listen_addr: SocketAddr, service_name: String, registry: Arc<BackendRegistry>) -> io::Result<()> {
...
pub async fn run_udp_gateway(listen_addr: SocketAddr, service_name: String, registry: Arc<BackendRegistry>) -> io::Result<()> {
...
- `tokio::net`: Async TCP and UDP socket handling.
- `copy_bidirectional`: Bi-directional stream forwarding.
- `UdpSocket`: Stateless request-response for packet proxying.
- `Arc`: Shares registry across tasks.
- `tokio::spawn`: Concurrent per-request/task handler.

🛠️ Notes:
- Backend targets are expected to be raw IP:PORT strings.
- Basic pass-through without protocol awareness.
*/
